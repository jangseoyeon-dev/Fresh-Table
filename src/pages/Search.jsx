  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
            ...
            {filters.foodTypes.map((type, idx) => (
              <button
                ...
                className={`px-4 py-2 text-sm rounded-full cursor-pointer ${
                  selectedFoodType === type ? 'bg-[#66BB6A] text-white' : 'bg-gray-100 hover:bg-gray-200'
                }`}
              >
                {type}
              </button>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {filters.cookingMethods.map((method, idx) => (
              <button
                key={idx}
                onClick={() => setSelectedCookingMethod((prev) => (prev === method ? null : method))}
                className={`px-4 py-2 text-sm rounded-full cursor-pointer ${
                  selectedCookingMethod === method ? 'bg-[#66BB6A] text-white' : 'bg-gray-100 hover:bg-gray-200'
                }`}
              >
                {method}
              </button>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {filters.calorie.map(({ label, value }) => (
              <button
                key={value}
                onClick={() => setCalorieFilter((prev) => (prev === value ? null : value))}
                className={`px-4 py-2 text-sm rounded-full cursor-pointer ${
                  calorieFilter === value ? 'bg-[#66BB6A] text-white' : 'bg-gray-100 hover:bg-gray-200'
                }`}
              >
                {label}
              </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
        ...
        {!isLoading && data?.length === 0 && <p className='font-semibold'>검색 내용에 해당하는 레시피가 없습니다.</p>}
        {!isLoading &&
          data?.map((recipe, idx) => (
            <div
              key={idx}
            ...
            >
              ...
              />
              <h3 className="mt-2 text-lg font-semibold text-gray-900 group-hover:underline">{recipe.RCP_NM}</h3>
