...
const RecipeCard = ({ title, image, category, onClick, isViewed = false }) => {
  return (
    <div className="relative overflow-hidden shadow-lg hover:scale-105 transition-transform cursor-pointer">
      {/* 이미지 */}
      <img src={image} alt={title} className="w-full h-64 object-cover" />

      {/* 좋아요 하트 버튼 (최근 본 레시피에서만 제거) */}
      {!isViewed && <LikeButton title={title} image={image} />}
      ...
      >
        <h3 className="text-lg font-bold w-[80%] drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">
          {title}
        </h3>
        {category && (
